...
print(
    f"  V_below / V_above ratio: {np.mean(Z_below[band])/np.mean(Z_above[band]):.3f} (should be ~1 for TEM)"
)
...
print("=" * 88)
print(
    f"{'Band':>14} {'f (GHz)':>9} {'Z₀ |V/I|':>10} {'|S21| PEC':>11} {'|S21| real':>12} {'IL/in real':>11}"
)
print("-" * 88)
...
for bn, kf in zip(band_names, key_freqs_ghz, strict=False):
    ...
    il = s21p - s21r
    print(
        f"{bn:>14} {freq[idx]/1e9:9.2f} {z_real:9.2f}Ω {s21p:10.2f}dB {s21r:11.2f}dB {il:10.3f}dB"
    )
