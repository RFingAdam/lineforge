...
is organized into submodules:
    ...
    atlc3.geometry      — Pydantic geometry models, rasterizer, Usermap
    atlc3.materials     — material database, MoreColors.txt loader, JSON packs
    atlc3.analytical    — closed-form solvers (Hammerstad-Jensen, Wadell)
    atlc3.solvers       — numerical kernels (Phase 2: C/Gp; Phase 3: L/Rs)
    atlc3.visualization — V/E/D/J/loss field rendering
    atlc3.scripting     — atlc2 .txt script-file interpreter
    atlc3.cli           — Typer CLI (entry point: ``atlc3``)
    atlc3.mcp_server    — MCP server (entry point: ``atlc3 mcp-serve``)

The native Rust kernels are accessed via :mod:`atlc3._kernel`.
...
Quick start::
    ...
    # Bitmap path:
    um = atlc3.from_bmp("usermap.bmp", pixel_width="0.1mm")
    cgp = atlc3.solve_cgp(um, frequency="1GHz")
"""
...
def solve(
    ...
    *,
    ...
    ----------
    geometry
        A model from :mod:`atlc3.geometry.types`, a :class:`Usermap`, or a dict
    ...
    from atlc3.solvers.dispatcher import solve as _dispatch
